...
def lade_buchungen():
    ...
    try:
        with open('data/buchungen.csv', newline='', encoding='utf-8') as file:
            ...
            for row in reader:
                if len(row) < 17:
                    row += [''] * (17 - len(row))  # ⬅️ ANPASSUNG: 14 statt 13 Spalten
                art, _, _, _, _, _, _, _, _, tischnummer, anzahl, _, _, _,_,_,_ = row  # ⬅️ ANPASSUNG: 14 Variablen
...
def speichere_buchungen(buchungen):
    with open('data/buchungen.csv', mode='w', newline='', encoding='utf-8') as file:
        ...
        writer.writerow([
            ...
            'Kommentar', 'Status', 'Betrag (€)', 'Zeitstempel', 'Mail gesendet', 'Tickets gesendet'  # ⬅️ NEUE SPALTE
        ])
